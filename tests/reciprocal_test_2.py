...
from fhelib.auxiliary.reciprocal_univ_guess import reciprocal_newton_universal_guess, reciprocal_partial_sums_geometric
...
print(f"z_max = {max(vals)}, convergence requires x0 < 2/{max(vals)} = {2/max(vals):.4f}")
print()
...
print()
print("=" * 50)
print("Test 4: Geometric series reciprocal (values near 1, |z-1| < 1)")
print("=" * 50)
...
for n_terms in [5, 10, 20, 50, 100, 500]:
    print(f"\nn={n_terms} terms:")
...
print()
print("=" * 50)
print("Test 4b: Accuracy vs n_terms (convergence rate)")
print("=" * 50)

v_single = Ciphertext(1)
v_single.set_element(0, 1.5)
true_val = 1 / 1.5
print(f"Approximating 1/1.5 (true = {true_val:.10f})")
print()
for n_terms in [1, 2, 5, 10, 20, 50, 100]:
    result = reciprocal_partial_sums_geometric(v_single, n=n_terms)
    print(_counts)
    reset()
    got = float(np.real(result[0]))
    print(f"  n={n_terms:>3}: {got:.10f}  error {abs(got - true_val):.2e}")

print()
print("=" * 50)
print("Test 4c: EXPECTED FAIL - value outside convergence zone |z-1| >= 1")
print("=" * 50)

# pad to 4
bad_vals = [2.0, 5.0, 0.0]
bad_vals_padded = bad_vals + [1.0]
v_bad = Ciphertext(4)
for idx, val in enumerate(bad_vals_padded):
    v_bad.set_element(idx, val)

print("These should diverge or be wildly inaccurate:")
result_bad4 = reciprocal_partial_sums_geometric(v_bad, n=50)
for i, val in enumerate(bad_vals):
    got = float(np.real(result_bad4[i]))
    expected = f"{1/val:.7f}" if val != 0.0 else "undefined"
    print(f"  1/{val}: got {got:.7f}  expected {expected}")

print()
print("=" * 50)
print("Test 4d: ValueError raised for out-of-range assumed_range")
print("=" * 50)

try:
    reciprocal_partial_sums_geometric(v4, n=10, assumed_range=(0.5, 2.5))
